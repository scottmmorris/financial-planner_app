...
class Division {
    ...
    // returns the fields of the entry (unnecessary, should be removed)
    getEntry(name) {
    ...
    // gets the id of an entry based on a name (unnecessary and potentially dangerous, should be removed)
    #getId(name) {
        for (const id in this.entries) {
            if (this.entries[id].name == name) return id;
        }
    }
